...
PORT=0
DEFCONFIG=lineageos
DEFCONFIG_S6EDGE=zeroltexx_defconfig
DEFCONFIG_S6FLAT=zerofltexx_defconfig
...
if [ $prompt == "1" ]; then
    ...
    TMOBILE=0
    LOG=$FLAT_LOG
    echo "S6 Flat G920F Custom Selected"
fi
...
# ---------
	...
	make -j$BUILD_JOB_NUMBER \
		"$DEFCONFIG"_"$KERNEL_DEFCONFIG" || exit -1
